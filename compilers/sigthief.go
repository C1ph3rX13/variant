...
func (ct *CertThief) SaveSignedPE(certTable []byte, outputFileName string) error {
	p := filepath.Join(ct.WorkingDir, ct.SrcFile)
	...
	outputFilePath := filepath.Join(ct.WorkingDir, outputFileName)
...
func (ct *CertThief) SaveCertificate() error {
	p := filepath.Join(ct.WorkingDir, ct.Target)
	certTable, err := ct.ReadCertTable(p)
	if err != nil {
		return err
	}

	certFilePath := filepath.Join(ct.WorkingDir, ct.CertFile)
...
func (ct *CertThief) SignWithStolenCert() error {
	pCert := filepath.Join(ct.WorkingDir, ct.CertFile) // 证书路径
...
func (ct *CertThief) SignExecutable() error {
	pFile := filepath.Join(ct.WorkingDir, ct.Target) // 待签名文件路径
